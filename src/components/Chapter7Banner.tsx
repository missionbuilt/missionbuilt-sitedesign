...
const Chapter7Banner = () => {
  ...
  useEffect(() => {
    ...
    const dismissed = localStorage.getItem('chapter7-banner-dismissed');
    if (!dismissed) {
      setIsVisible(true);
    }
  }, []);
  ...
  const handleDismiss = () => {
    setIsVisible(false);
    localStorage.setItem('chapter7-banner-dismissed', 'true');
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-foreground">
                <span className="text-sunburst font-bold">Chapter 7 out now:</span>{' '}
                Train the Engine, Not Just the Output
              </p>
            </div>
          </div>
